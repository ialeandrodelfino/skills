...
2. Use `assets/SKILL.template.md` only as a starting point; keep the sections the task needs. Give the package a matching lowercase hyphenated `name` and a concise `description`. Treat 1,024 characters as a validation ceiling, not a writing target; preserve harness-specific metadata requirements.
