...
- **Producing Firecrawl-powered deliverables such as research briefs, SEO audits, QA reports, lead lists, knowledge bases, or design-system extraction** -> use the `firecrawl-workflows` skills (available separately with `firecrawl setup workflows`). These skills infer from context first and ask only short blocking questions when needed.
